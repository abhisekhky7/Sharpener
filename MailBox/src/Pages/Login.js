...
import { saveUserEmail } from '../store/MailSlice';
...
export default function Login() {
  ...
  const handleSubmit=(event)=>{
        ...
        if(!isLoggedIn && pass!==confirmPass){
        ...
        let url;
        if(isLoggedIn){
            url='https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyC_fWmOCyaTQztVt6aDzNwfEbN4wCvpMFU'
        }else{
          url="https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyC_fWmOCyaTQztVt6aDzNwfEbN4wCvpMFU"
        }
        fetch(url,{
            method:"POST",
            body:JSON.stringify({
  ...
  return (
    ...
    <div  className="d-flex flex-column myDiv text-center bg-dark mx-auto px-2"  style={{height:isLoggedIn?"55vh":"65vh"}}>
      ...
      <form onSubmit={handleSubmit} className=' px-2 text-center'>
       ...
       { !isLoggedIn && <div>
        <label className='text-white my-2' htmlFor='username'>Confirm Password:</label><br/>
        <input className='w-100' type='password' required id='confirmPassword' value={confirmPass} onChange={(e)=>setConfirmPass(e.target.value)}/>
        </div>
    }
        ...
        <button type='button'
         ...
         className='mt-3 btn btn-danger logintext border border-md-white border-white'>
